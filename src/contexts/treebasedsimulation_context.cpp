...
TreeBasedSimulationContext::TreeBasedSimulationContext(AreaData *area, const BaseReactorContext *reactor, float levels) :
...
//    if (!_tree.diagnostic()) return EventInfoData(0);
    ...
    while (true) {
        ...
        NodeSite *currentNode = _tree.find(&r);
        if (currentNode == 0) {
            std::cerr << "Node not found! -> _tree.reCount()" << std::endl;
            isRealyOver = true;
            _tree.reCount(this);
            continue;
        }
